...
import { createFeedback, getFeedbackByCourse, getFeedbackByStudentAndCourse, updateFeedback, deleteFeedback, getCourseRatingSummary, } from './feedback.service.js';
const router = Router();
/**
 ...
 * @access  Private (requires authentication)
 */
router.get('/my-feedback/:courseId', authenticate, async (req, res) => {
    try {
        const studentId = req.user.id;
        const { courseId } = req.params;
